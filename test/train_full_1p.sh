...
taskset -c 0-23 python3.7 train.py --data data/coco.yaml --cfg models/yolov5x.yaml --weights '' --batch-size ${batch_size} --device npu --npu ${ASCEND_DEVICE_ID}  --epochs ${train_epochs} >> ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
